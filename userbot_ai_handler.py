...
async def enforce_moderation(client: Client, msg: Message, text: str) -> bool:
...
async def _transcribe_audio(msg) -> str | None:
    try:
        ...
        if getattr(target, 'voice', None):
            byte_array = await client.download_media(target, in_memory=True)
            if byte_array:
...
async def _send_voice_reply_if_needed(client, msg, reply_text, prompt, is_voice_input=False) -> bool:
    import random
    import time
    import httpx
    import io
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    await _log_ai_usage(client, msg, prompt, f"[Voice Note Generated]\n{reply_text}")
                    return True
        except Exception as e:
...
async def cmd_speak(client: Client, msg: Message) -> None:
    ...
    reply_text = await generate_ai_response(history, is_owner=is_owner, is_gf=is_gf, client=client, msg=msg)
    await _append_ai_response(msg.chat.id, reply_text)
...
async def _log_ai_usage(client, msg, prompt, reply):
    ...
    try:
        ...
        await client.send_message(chat_id=log_chat_id, text=log_text, parse_mode=ParseMode.HTML)
    except Exception as e:
...
async def _extract_base64_image(msg) -> str | None:
    try:
        if msg.photo:
            byte_array = await client.download_media(msg, in_memory=True)
            if byte_array:
                byte_array = byte_array.getvalue()
            return base64.b64encode(byte_array).decode('utf-8')
        elif msg.reply_to_message and msg.reply_to_message.photo:
            byte_array = await client.download_media(msg.reply_to_message, in_memory=True)
            if byte_array:
                byte_array = byte_array.getvalue()
            return base64.b64encode(byte_array).decode('utf-8')
    except Exception:
...
async def _check_and_execute_raw_tool_call(content: str, client, msg) -> str:
    if not content:
        return ""
    import json
    ...
    if match:
        ...
        try:
            ...
            if func_name == "moderate_user" and client and msg:
                tool_res = await execute_moderation_tool(client, msg, args.get("action"), args.get("duration_minutes", 0))
...
async def cmd_summary(client: Client, msg: Message) -> None:
...
async def cmd_roast(client: Client, msg: Message) -> None:
    ...
    elif (msg.command[1:] if hasattr(msg, "command") and msg.command else []):
        target_name = " ".join((msg.command[1:] if hasattr(msg, "command") and msg.command else []))
    else:
        target_name = "them"
...
async def execute_moderation_tool(client, msg, action: str, duration_minutes: int):
    ...
    try:
        if action == "mute":
            ...
            await client.unban_chat_member(chat_id, target_user.id, only_if_banned=True)
...
async def generate_ai_response(history: list[dict], base64_image: str = None, is_owner: bool = False, is_gf: bool = False, client=None, msg=None, override_system_prompt: str = None, force_model: str = None) -> str:
    ...
    tools = None
    tool_choice = "none"
    if is_owner and client and msg and msg.reply_to_message:
        tools = [
            {
                "type": "function",
                "function": {
    ...
    if tools and groq_client:
        try:
            ...
            if message.tool_calls:
                ...
                tool_result = await execute_moderation_tool(client, msg, args.get("action"), args.get("duration_minutes", 0))
            ...
            return await _check_and_execute_raw_tool_call(message.content or "", client, msg)
        except Exception as e:
...
async def cmd_ai(client: Client, msg: Message) -> None:
    """/ai <prompt> command"""
...
async def check_auto_ai(client: Client, msg: Message) -> None:
    ...
    # Mention heuristic
    is_mention = client.me.username and f"@{client.me.username}" in text
    ...
    if is_mention or is_reply_to_bot or is_question or msg.chat.type.name == 'PRIVATE':
        ...
        if client.me.username:
            prompt = prompt.replace(f"@{client.me.username}", "").strip()
        ...
        reply = await generate_ai_response(history, base64_image=img, is_owner=is_owner, is_gf=is_gf, client=client, msg=msg)
